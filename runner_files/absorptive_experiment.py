...
directory = "results/masters5/final_data_O_COUNTER_4PAM/"
...
params = {
    ...
    "AVG_POWER": [1,2,3,4,5,6,7,8], # Here, you input a list of average powers (dBm, check this) that you want to iterate over. When the results are printed out, they'll contain only the avg_power that was used in that iteration. 
...
if __name__ == "__main__":
    ...
    for power in avg_powers: 
        ...
        seeds = [1*np.random.rand()*present_time, 2*np.random.rand()*present_time, 3*np.random.rand()*present_time, 4*np.random.rand()*present_time]
        ...
        params_filename = directory+f"params{round(10*np.log10(1000*power))}.json"
        file_pointer = open(params_filename, 'w')
        dump(params, file_pointer)
        file_pointer.flush()

        Path("results").mkdir(parents=True, exist_ok=True)
        filename = directory+f"absorptive{round(10*np.log10(1000*power))}.json"
        fh = open(filename, 'w')
