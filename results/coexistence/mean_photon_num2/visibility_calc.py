...
def plot_interference(phases, freq_0, st_dev_0, freq_1, st_dev_1, params_0, params_1, directory, power, mpn):
    ...
    ax.legend()
    fig.tight_layout()
    plt.savefig(directory + f'interference{mpn}_{int(power)}.png')
    plt.show()
...
final_data.writerow(["power", "mpn", "visibility"])
pattern = "\d[.]?\d*"
...
for params_file_name in os.listdir():
    ...
    if params_file_name[:6] == "params":
        ...
        try:
            ...
            values = re.findall(pattern, params_file_name)
            mpn = float(values[0])
        ...
        # if power == None:
        #     power = "No_Power"
        final_data_entry = [power, mpn]

        try:
            print("file name is:", f"absorptive{mpn}_{int(power)}.json")
            data_file_name = directory + f"absorptive{mpn}_{int(power)}.json"
        ...
        plot_interference(np.linspace(0, 2*np.pi, num=data["num_phase"]), freq_0, st_dev_0, freq_1, st_dev_1, params_0, params_1, directory, power, mpn)
