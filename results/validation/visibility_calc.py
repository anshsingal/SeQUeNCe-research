...
final_data.writerow(["efficiency", "visibility"])


for params_file_name in os.listdir():
    ...
    if params_file_name[:6] == "params":
        ...
        try:
            ...
            efficiency = float(params_file_name[6:-5])
        except:
            continue
        ...
        # if power == None:
        #     power = "No_Power"
        final_data_entry = [efficiency]

        try:
            print("file name is:", f"Edistribution{efficiency}.json")
            data_file_name = directory + f"Edistribution{efficiency}.json"
            data = load(open(data_file_name))
        except:
            continue


# directory = "results/final_data_O_CO_4PAM/"
...
#             continue
        ...
        plot_density_matrix(bins, off_diag, sin, directory, efficiency)
        plot_interference(np.linspace(0, 2*np.pi, num=data["num_phase"]), freq_0, st_dev_0, freq_1, st_dev_1, params_0, params_1, directory, efficiency)
...
#     return freq_0, st_dev_0, freq_1, st_dev_1, params_0, params_1, vis_0, vis_1, vis_total


# def plot_interference(phases, freq_0, st_dev_0, freq_1, st_dev_1, params_0, params_1, directory, efficiency):
#     # plotting interference
#     fig = plt.figure(figsize=(8, 5))
#     ax = fig.add_subplot()
...
#     ax.legend()
#     fig.tight_layout()
#     plt.savefig(directory + f'interference_{efficiency}.png')
#     plt.show()
...
#         def sin(x, A, phi, z): return A*np.sin(x + phi) + z
#         freq_0, st_dev_0, freq_1, st_dev_1, params_0, params_1, vis_0, vis_1, vis_total = analyze_data(data, sin)
#         # final_data_entry.extend(bins)
#         final_data_entry.append(vis_total)
#         # final_data_entry.append(off_diag)
