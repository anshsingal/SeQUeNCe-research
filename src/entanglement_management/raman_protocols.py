...
class RamanTestSender(Protocol):
    """ Sender's protocol"""
    def __init__(self, own, num_iterations):
        self.own = own
    ...
    def start(self, classical_communication):
        ...
        ############## frequency of the optical channel and not the sender itself. 
        
        print(type(self.own.timeline.now()))
...
class RamanTestReceiver(Protocol):
    ...
    def __init__(self, BSM, pulse_separation):
    ...
    def stop_sim_and_process(self, hist_width):
        ...
        print("idler buffer:", len(self.idler_buffer))
        ...
        self.get_correlations(self.signal_buffer, self.idler_buffer, hist_width, None, None)
