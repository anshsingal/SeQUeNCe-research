    ...
    def load(self, rule: "Rule") -> bool:
        ...
        for memory_info in self.memory_manager:
            ...
            if len(memories_info) > 0:
                print(rule.action, rule.get_reservation().initiator, rule.get_reservation().responder, self.owner.name)
                rule.do(memories_info)
                for info in memories_info:
                    # print("success")
                    # print("setting rules for mem index:", info.index, 'of node:', self.owner.name, "with initiator:", rule.get_reservation().initiator, "to destination:", rule.get_reservation().responder)
                    info.to_occupied()
