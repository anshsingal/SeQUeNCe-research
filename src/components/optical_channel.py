...
class QuantumChannel(OpticalChannel):
    ...
    def __init__(self, name: str, timeline: "Timeline", attenuation: float, distance: int, polarization_fidelity=1, light_speed=2e-4, frequency=8e7, optical_wavelength = 1536):
        ...
        self.optical_wavelength = optical_wavelength
        self.delay = 0
        self.loss = 1
    ...
    def start_clock(self):
        ...
        raman_power = np.abs(clock_power * raman_coefficient * narrow_band_filter_bandwidth * (np.exp(-self.attenuation * self.distance) - np.exp(-clock_attenuation * self.distance)) / (self.attenuation - clock_attenuation))
        num_photons_added = int(raman_power / (h * c / self.optical_wavelength))
        raman_photon_distance_offset = np.floor((self.distance * np.random.rand(num_photons_added) / (3 * 10**5)) * 1e12)
        raman_photon_time_offset = np.floor(1e12*np.random.rand(num_photons_added))
        ...
        process = Process(self, "start_clock", [])
        event = Event(self.timeline.now() + 1e12, process)
        self.timeline.schedule(event)
