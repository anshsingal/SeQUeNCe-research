...
main(int argc, char* argv[])
{
    ...
    // Basically, Don't use any of this shite.
    Config::SetDefault("ns3::TcpL4Protocol::SocketType", StringValue("ns3::TcpNewReno"));
    Config::SetDefault("ns3::TcpSocket::InitialCwnd", UintegerValue(1));
    Config::SetDefault("ns3::TcpL4Protocol::RecoveryType",
                       TypeIdValue(TypeId::LookupByName("ns3::TcpClassicRecovery")));

    // Start from here. Creating the node objects.
    NodeContainer nodes;
    nodes.Create(2);

    // Point to Point helper. Usual stuff here. 
    PointToPointHelper pointToPoint;
    pointToPoint.SetDeviceAttribute("DataRate", StringValue("5Mbps"));
    pointToPoint.SetChannelAttribute("Delay", StringValue("2ms"));

    // Install NetDevices on the nodes. Usual stuff. 
    NetDeviceContainer devices;
    devices = pointToPoint.Install(nodes);

    // We want to use the Bit error model. You can set the ErrorUnit to "Bit" and try how does it go. Try this. 
    ...
    // Address helpers. Usual stuff again. 
    Ipv4AddressHelper address;
    address.SetBase("10.1.1.0", "255.255.255.252");
    Ipv4InterfaceContainer interfaces = address.Assign(devices);

    // Also done in previous implementation. Sink port where our packets would be sent. 
